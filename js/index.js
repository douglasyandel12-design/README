...
(function applyDarkMode(){const isDark=localStorage.getItem('lvs_dark_mode')==='true';if(isDark)document.documentElement.classList.add('dark-mode');})();
...
function updateModalPriceDisplay(product, quantity) {
    ...
    if (canSeeProgressivePromo && quantity >= 2) {
         ...
         html += `<small style="color: #d97706; display:block; font-size:0.8rem; margin-top:5px;">🔥 ¡Ahorras ${currencyManager.format(discount)} por unidad!</small>`;
    }
...
async function renderSingleProductPage(id) {
    ...
    singleContainer.innerHTML = `
        <style>
            ...
            .sp-description .ql-size-small { font-size: 0.75rem; }
            .sp-description .ql-size-large { font-size: 1.05rem; }
            .sp-description .ql-size-huge { font-size: 1.25rem; }
        ...
        <div class="sp-layout">
            ...
            <div class="sp-details">
                <nav style="margin-bottom: 1.5rem;">
                    <button onclick="window.closeSingleProductPage()" class="btn btn-outline" style="display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; border-radius: 8px; font-weight: 600; font-size: 1rem; border-width: 2px; cursor: pointer; width: auto;">
                        <span style="font-size: 1.2rem; line-height: 1;">←</span> Volver a la tienda
                    </button>
                </nav>
...
window.updateSpPriceDisplay = function(quantity) {
    ...
    if (!priceEl || !product) return;
    const unitPrice = calculateItemPrice(product, quantity);
    let html = `<span style="font-weight:500; color:#111;">${currencyManager.format(unitPrice)}</span>`;
    if (unitPrice < product.price) {
        html = `<span style="text-decoration:line-through; color:#9ca3af; margin-right:8px; font-size: 0.95rem;">${currencyManager.format(product.price)}</span> <span style="font-weight:600; color:#111;">${currencyManager.format(unitPrice)}</span>`;
    }
    if (product.discount > 0) html += ` <span style="background:#ef4444; color:white; padding: 2px 6px; border-radius: 4px; font-size:0.75rem; vertical-align: middle; margin-left: 8px; font-weight: 500;">-${product.discount}%</span>`;
...
function updateCartUI() {
    ...
    if (modalFooter) {
        ...
        if(modalTotal) modalTotal.textContent = currencyManager.format(total);
    }
}
